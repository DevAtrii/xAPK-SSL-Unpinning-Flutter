...
# Define paths
tmp_dir = 'tmp_flutter/xapk'
unsigned_dir = 'tmp_flutter/unsigned'
apk_dir = 'tmp_flutter/apk'
...
def get_uber_apk_signer_path():
    ...
    if 'uber_apk_signer' not in config:
        ...
        config['uber_apk_signer'] = signer_path
        save_config(config)
...
def cleanup_after_reflutter(chosen_apk, original_xapk_name):
...
def move_apks(original_xapk_name):
...
def cleanup():
    if os.path.exists('tmp_flutter'):
        shutil.rmtree('tmp_flutter')

# Main function
def main(xapk_path):
    original_xapk_name = os.path.basename(xapk_path).replace('.xapk', '')
    extract_xapk(xapk_path)
    chosen_apk = choose_apk()
    ip_addr = print_ip_address()

    # Run reflutter and provide the IP address when prompted
    run_reflutter(chosen_apk)
    print("Enter your IP address when prompted by reflutter.")
    
    cleanup_after_reflutter(chosen_apk, original_xapk_name)
    sign_apk_files()
    move_apks(original_xapk_name)
    cleanup()
...
if __name__ == "__main__":
    ...
    #     print(f"{xapk_path} does not exist.")
    #     exit(1)
    if len(sys.argv) < 2:
    ...
    # Extract the original XAPK filename without the extension for later use
